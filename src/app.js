...
Ammo().then((Ammo) => {
	...
	function createGridPlane() {
		...
		let blockPlane = new THREE.Mesh(
			...
			new THREE.MeshPhongMaterial({
				...
				transparent: true,
				opacity: 1,
				map: loader.load('./src/jsm/other/border.jpeg'),
			}),
			new THREE.MeshPhongMaterial({
				color: 0xffffff,
				transparent: true,
	...
	function createMan() {
		...
		fbxLoader.load('./src/jsm/user/Salute.fbx', (object) => {
			mixer = new THREE.AnimationMixer(object)
			for (let index = 0; index < object.animations.length; index++) {
				action[index] = mixer.clipAction(object.animations[index])
			}
			const AnimationAction = action[0]
			AnimationAction.timeScale = 10
			// AnimationAction.loop = THREE.LoopOnce //不循环播放
			AnimationAction.clampWhenFinished = true
			AnimationAction.play()

			object.position.set(6, 0, -60)
	...
	function createMan1() {
		...
		fbxLoader.load('./src/jsm/user/Patting.fbx', (object) => {
			mixer1 = new THREE.AnimationMixer(object)
			for (let index = 0; index < object.animations.length; index++) {
				action1[index] = mixer1.clipAction(object.animations[index])
			}
			console.log(action1)
			const AnimationAction = action1[0]
			AnimationAction.timeScale = 250
			// AnimationAction.loop = THREE.LoopOnce //不循环播放
			AnimationAction.clampWhenFinished = true
			AnimationAction.play()

			object.position.set(-50, 0, -30)
	...
	// create ball
	function createBall() {
		let pos = { x: 8.75, y: 0, z: 70 }
		let radius = 2
		let quat = { x: 0, y: 0, z: 0, w: 1 }
		let mass = 3

		var marble_loader = new THREE.TextureLoader(manager)
		var marbleTexture = marble_loader.load('./src/jsm/z.jpeg')
	...
	function createBox(
		...
		linkBox.rotateX(rotateX)
		linkBox.rotateY(rotateY)
		linkBox.rotateZ(rotateZ)
	...
	function loadFloydText() {
		...
		text_loader.load('./src/jsm/WDYRT_Regular.json', function (font) {
			var xMid, text
			...
			var geometry = new THREE.TextGeometry('叮咚买菜·不卖隔夜菜', {
				font: font,
				size: 6,
				height: 4,
				curveSegments: 12,
			...
			xMid = -0.35 * (geometry.boundingBox.max.x - geometry.boundingBox.min.x)
	...
	function createBillboardActive(
		x,
		y,
		z,
		textureImage = billboardTextures.grassImage,
		...
		const billboardPoleScale = { x: 1, y: 25, z: 1 }
		const billboardSignScale = { x: 40, y: 25, z: 1 }
		...
		billboardSign.position.x = x
		billboardSign.position.y = 15
		billboardSign.position.z = z

		/* Rotate Billboard */
		// billboardPole.rotation.y = rotation
		billboardSign.rotation.z = Math.PI / 2
		billboardSign.rotation.x = -Math.PI / 2
		billboardSign.rotation.y = Math.PI / 4
	...
	function start() {
		...
		// createBox(12, 2, -70, 5, 5, 1, boxTexture.c1, URL.gitHub, 0x000000, false)

		// createBox(12, 10, -70, 5, 5, 1, boxTexture.c1, URL.gitHub, 0x000000, false)

		// createBox(22, 2, -70, 5, 5, 1, boxTexture.c1, URL.twitter, 0x000000, false)
		// createBox(22, 10, -70, 5, 5, 1, boxTexture.c1, URL.twitter, 0x000000, false)
		...
		// createBox(42, 2, -70, 5, 5, 1, boxTexture.c1, URL.devTo, 0x000000, false)

		const boxCTextureResult = []
		const boxCTextureResultR = []
		for (let i = 0; i < boxCTexture.length; i += 8) {
			boxCTextureResult.push(boxCTexture.slice(i, i + 8))
		}

		for (let i = 0; i < boxCTexturer.length; i += 8) {
			boxCTextureResultR.push(boxCTexturer.slice(i, i + 8))
		}

		boxCTextureResult.forEach((item, index) => {
			item.forEach((i, indexz) => {
				createBox(
					12 + indexz * 10,
					2 + index * 8,
					-70,
		...
		boxCTextureResultR.forEach((item, index) => {
			item.forEach((i, indexz) => {
				createBox(
					82,
					3 + index * 10,
		...
		// floatingLabel(12.875, 14.5, -70, '大白菜\n5星币 / 500g ')
		// floatingLabel(22.875, 6.5, -70, '大白菜\n5星币 / 500g ')
		// floatingLabel(22.875, 14.5, -70, '大白菜\n5星币 / 500g ')
		// floatingLabel(32.875, 6.5, -70, '大白菜\n5星币 / 500g ')
		// // floatingLabel(35, 6.5, -70, '  Static \nWebsite');
		// floatingLabel(42.875, 6.5, -70, '大白菜\n5星币 / 500g ')
		// floatingLabel(44, 6.5, -70, '   How I \nmade this');

		// allSkillsSection(-50, 0.025, 20, 40, 40, boxTexture.allSkills);

		createBillboardActive(
			-71,
			2.5,
			18,
			'./src/jsm/other/t.webp',
			'https://www.100.me/home/index',
			Math.PI * 0.22
		)
		allSkillsSection(
			...
			11,
			10,
			'./src/jsm/other/k.jpeg',
			null,
			-Math.PI / 4,
			0,
			0
		)
		floatingLabel(8, 8.1, 65, `模态球启动 \n 按键移动`, -Math.PI / 4, 0, 0)
		...
		if (isTouchscreenDevice()) {
			touchText = '请随意挑选'
			instructionsText = '欢迎来到叮咚买菜元宇宙总店，开始生鲜之旅!'
		} else {
			touchText = '请随意挑选'
			instructionsText = '欢迎来到叮咚买菜元宇宙总店，开始生鲜之旅!'
		}

		simpleText(9, 0.01, 45, instructionsText, 3)

		simpleText(23, 0.01, -60, touchText, 2)
		// simpleText(-50, 0.01, -5, 'SKILLS', 3);
		simpleText(-42, 0.01, -50, '智能导控', 3)
		simpleText(-61, 0.01, -15, '生鲜未来', 3)
		...
		createTriangle(12, -26)
		createTriangle(12, -18)
		createTriangle(12, -10)
		createTriangle(12, -2)
		createTriangle(12, 6)
		createTriangle(12, 14)
		createTriangle(12, 22)
		createTriangle(12, 30)
